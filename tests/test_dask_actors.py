...
def test_country_actor_update():
    ...
    try:
        ...
        # Run update
        cell_indices, to_earth_updates, individual_updates = actor.update(
            t=2025
        )
        ...
        np.testing.assert_allclose(
            to_earth_updates["pft_harvest_frac"], np.full(3, 202.5)
        )
    finally:
        ser.deserialize_country = original_deserialize
...
def test_country_actor_from_earth_update():
    ...
    try:
        ...
        np.testing.assert_allclose(
            actor.country._world.from_earth.soilc.values, [10.0, 20.0, 30.0]
        )
    finally:
        ser.deserialize_country = original_deserialize
...
def test_country_actor_broadcast_update():
    ...
    try:
        ...
        np.testing.assert_allclose(
            slice_result["soilc"], [200.0, 300.0]  # Values at indices 1 and 2
        )

        # Run update with broadcast (returns result tuple)
        result = actor.update_with_broadcast(
            t=2025, from_earth_full=from_earth_full, cell_indices=cell_indices
        )
